...
function loadMessages() {
  ...
  if (messages.length === 0) {
    container.innerHTML = '<p class="text-muted text-center">Belum ada pesan di forum.</p>';
    return;
  }
  ...
  messages.forEach(msg => {
    ...
    msgDiv.innerHTML = `<strong>${escapeHtml(msg.username)}</strong> <small class="text-muted">[${new Date(msg.timestamp).toLocaleString()}]</small><br>${escapeHtml(msg.text)}`;
